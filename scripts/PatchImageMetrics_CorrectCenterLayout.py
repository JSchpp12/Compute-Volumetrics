...
class MarchedInfo:
    ...
    cutoffValue : Optional[float] = None
...
def create_correct_frame_from_bad_dict(data : dict) -> Frame: 
    x = data["terrain_shape"]["center_lat"]
    y = data["terrain_shape"]["center_lon"]

    vis = None
    if "vis_range" in data["terrain_shape"]:
        vis = data["terrain_shape"]["vis_range"]
    else:
        vis = data["terrain_shape"]["view_distance"]

    terrain_shape : TerrainShapeFile = TerrainShapeFile(center=Vec2(x, y), view_distance=vis)
    data["terrain_shape"] = asdict(terrain_shape)
    return Frame.from_dict(data)

def process_image_file(img_data : ImageToUpdate) -> None: 
    corrected_data = None
    with open(img_data.path, 'r') as f:
        corrected_data = create_correct_frame_from_bad_dict(json.load(f))
    with open(img_data.path, 'w') as f:
        if corrected_data is not None:
            json.dump(asdict(corrected_data), f, indent=4)
...
def main(): 
    """_summary_
    This script corrects the following layout issue in the provided image metric json files: 

    A previous pass of patches applied "center_lat" and "center_lon" to the center paramter in the metric files.
    This will correct those to 'x' and 'y' to follow new layout of metric files.
    """
