...
VolumeShadowRenderPhaseProvider::VolumeShadowRenderPhaseProvider(
...
std::unique_ptr<star::core::renderer::RenderPhase> VolumeShadowRenderPhaseProvider::build(
    star::core::device::DeviceContext &c, star::core::renderer::RenderPhaseRegistry &phases)
{
    ...
    auto phase = std::make_unique<VolumeShadowRenderPhase>(m_enableShadowCasting);
    ...
    assert(offscreenPhase != nullptr && "offscreen render phase must be built before the volume shadow render phase");
    ...
    phase->m_commandBuffer = c.getManagerCommandBuffer().submit(
        star::core::device::manager::ManagerCommandBuffer::Request{
            .recordBufferCallback = std::bind(&VolumeShadowRenderPhase::recordCommandBuffer, phase.get(),
