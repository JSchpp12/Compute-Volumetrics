...
void ImageMetricManager::init()
{
    ...
    m_copier.prepRender(*m_device, *m_eb, *m_cb, *m_qm, *m_frameTracker);
...
void ImageMetricManager::recordThisFrame(const Volume &volume, const std::string &imageCaptureFileName)
{
    ...
    m_copier.trigger(*m_cb, *rayAtCutoff, *rayDistance, volume.getRenderer().getRayAtCutoffBufferAt(fi),
    ...
    auto writeToFile = image_metric_manager::FileWriteFunction(hostResource, m_device->getVulkanDevice(),
                                                               semaphoreRecord->semaphore, 0, &m_storage);
    ...
    // properly sync screen capture service cmds
    {
