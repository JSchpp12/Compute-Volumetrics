...
std::shared_ptr<StarScene>
Application::createInitialScene(StarDevice &device, const StarWindow &window,
                                const uint8_t &numFramesInFlight) {
  std::shared_ptr<star::BasicCamera> camera =
      std::make_shared<star::BasicCamera>(window.getExtent().width,
                                          window.getExtent().height, 90.0f,
                                          0.1f, 20000.0f, 500.0f, 0.1f);

  camera->setPosition(glm::vec3{4.0f, 0.0f, 0.0f});
  camera->setForwardVector(glm::vec3{0.0, 0.0, 0.0} - camera->getPosition());

  std::shared_ptr<StarScene> newScene =
      std::make_shared<star::StarScene>(numFramesInFlight, camera);

  return newScene;
}

void Application::startup(star::StarDevice &device,
                          const star::StarWindow &window,
                          const uint8_t &numFramesInFlight) {
  auto mediaDirectoryPath =
      star::ConfigFile::getSetting(star::Config_Settings::mediadirectory);
  auto horsePath = mediaDirectoryPath + "models/horse/WildHorse.obj";

  {
    int framesInFLight = std::stoi(
        star::ConfigFile::getSetting(star::Config_Settings::frames_in_flight));
    ...
    this->offscreenScene = std::make_shared<star::StarScene>(
        framesInFLight, this->scene->getCamera(), globalInfos, lightInfos);
    this->offscreenSceneRenderer =
        std::make_unique<OffscreenRenderer>(this->offscreenScene);
    ...
    auto screen = std::make_unique<Volume>(
        this->scene->getCamera(), width, height, this->scene->getLights(),
        this->offscreenSceneRenderer->getRenderToColorImages(),
        this->offscreenSceneRenderer->getRenderToDepthImages(), globalInfos,
        lightInfos);
  ...
  this->offscreenScene->add(std::make_unique<star::Light>(
      star::Type::Light::directional, glm::vec3{0, 10, 0},
  ...
  this->scene->add(std::make_unique<star::Light>(star::Type::Light::directional,
                                                 glm::vec3{0, 10, 0},
                                                 glm::vec3{-1.0, 0.0, 0.0}));
...
void Application::onKeyRelease(int key, int scancode, int mods) {
  ...
  if (key == star::KEY::SPACE) {
    ...
    this->testObject->setPosition(glm::vec3{
        camPosition.x + (static_cast<float>(
                            MathHelpers::MilesToMeters(camLookDirection.x))),
        camPosition.y + (static_cast<float>(
                            MathHelpers::MilesToMeters(camLookDirection.y))),
        camPosition.z + (static_cast<float>(MathHelpers::MilesToMeters(
                            camLookDirection.z * MILES_TO_METERS)))});
  }
  ...
  if (key == star::KEY::B) {
    ...
    switch (selectedMode) {
    case (1):
      this->vol->getFogControlInfo().linearInfo.nearDist =
          PromptForVisibilityDistance();
      break;
    case (2):
      this->vol->getFogControlInfo().linearInfo.farDist =
          PromptForVisibilityDistance();
      break;
    case (3):
